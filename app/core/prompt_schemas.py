...
class ExecutionDisciplineSchema(LLMVisibleSchema):
    ...
    def to_llm_markdown(self) -> str:
        ...
        return (
            ...
            "4. **No actionable task ⇒ no exploration. BUT user direct "
            "instruction IS an actionable task — even if it isn't a "
            "formal dispatch.** If the user message contains imperative "
            "verbs (做 / 写 / 改 / 看 / 找 / 保存 / 上传 / "
            "fetch / save / write / build / fix / search) and a clear "
            "object, **EXECUTE the instruction directly**. DO NOT call "
            "`project_state` / `check_inbox` / `sc_query` to \"first "
            "see what's pending\" — there is no pending dispatch, the "
            "user IS the dispatch. Only fall back to "
            "\"等待具体任务指令\" when the trigger is genuinely vague "
            "(\"check inbox\" / \"继续\" / \"工作\" with no object) AND "
            "your inbox is empty AND the user message has no imperative.\n"
            ...
            "9. **Lessons / retros / playbooks MUST land in the wiki.** "
            "If the file you just wrote is a retrospective report, "
            "lessons-learned doc, project-experience playbook, or "
            "methodology guide (filenames containing `复盘` / `retro` / "
            "`lessons` / `经验` / `playbook` / `启动手册` / `methodology`), "
            "you MUST follow the write_file with a wiki_ingest call so "
            "future agents on new projects can `knowledge_lookup` and "
            "find it. A file on disk alone won't be auto-recalled — "
            "your peer agent next month doesn't know the path. Example:\n"
            "    wiki_ingest(\n"
            "      kind='experience',\n"
            "      title='项目复盘 — RPG POC GAME of ME',\n"
            "      body=<the markdown you just wrote>,\n"
            "      tags=['retrospective','agent-collaboration','poc'],\n"
            "      scope='global'   # or omit for role-scoped\n"
            "    )\n"
            "10. **工具失败必须复述真实错误,不准编造解释.** When a "
            "tool call fails, your reply MUST quote the ACTUAL error "
            "message (including sandbox deny / permission denied / "
            "network timeout / 404 / connection refused / etc.) "
            "verbatim. **DO NOT** rewrite a sandbox / permission "
            "error as \"配置缺失\" / \"未安装\" / \"环境问题\" / "
            "\"file not found\" / \"missing dependency\" — those "
            "fabrications send the user debugging the wrong way. "
            "Concrete examples of forbidden translations:\n"
            "  ✗ Real error: 'Sandbox violation: /Users/foo/bar.md "
            "outside jail' → DO NOT report '缺少配置文件' or '文件不存在'\n"
            "  ✗ Real error: 'Permission denied' → DO NOT report '未安装'\n"
            "  ✗ Real error: '402 Payment Required' → DO NOT report "
            "'API 不可用'\n"
            "If you genuinely don't understand an error, paste it "
            "verbatim and ask the admin/user. Honest 'I hit X, what "
            "does that mean?' beats invented 'must be Y' every time.\n"
            "</execution_discipline>"
        )
