...
来源
----
Port 自 openai-agents-python (MIT,
https://github.com/openai/openai-agents-python) 的
``src/agents/apply_diff.py``,逐字保留实现 — 该解析器在 OpenAI 的 codex /
代码编辑 agent 里实战验证过,边角情况处理(模糊匹配 / EOF 锚点 /
CRLF 归一化)非常细致。
...
两种语法,通过 ``mode`` 选择:
  ...
  - ``mode="default"`` — Update File,带上下文 hunk::
...
锚点 (``@@ ...``) 让解析器先把游标推到 anchor 行下面,再用上下文 hunk
精确匹配修改位置;允许 trailing-whitespace / 全行 strip 的模糊回退,但会
通过 ``fuzz`` 计数器累计模糊度,调用方可以判断是否要拒绝过度模糊的 patch。

谁该用这个库
------------
TudouClaw 目前 ``write_file`` 工具是整文件覆盖,大文件改一行也得 LLM 重输
整份内容,既费 token 也容易出错。``apply_diff`` 提供结构化局部编辑能力,
适合接到:

  * 未来的 ``apply_patch`` / ``edit_file`` 工具
  * coding 类 skill 的批量改写流程
  * verifier 阶段对比 LLM 输出和 ground-truth diff

故意不直接 wire 到现有工具 — 加载 LLM 工具集需要谨慎(参数 schema /
denylist / approval),由调用方按需引入。
...
::
    ...
    new_text = apply_diff(old_text, diff_str, mode="default")
    # 或新建文件:
    new_text = apply_diff("", diff_str, mode="create")

异常
----
解析失败时抛 ``ValueError`` 并附带 patch 里的违规行,调用方应捕获并把
原始错误透传给 LLM(这样 LLM 能基于错误自纠)。
"""
...
__all__ = ["apply_diff", "ApplyDiffMode"]
